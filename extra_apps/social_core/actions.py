...
def do_complete(backend, login, user=None, redirect_name='next',
    ...
    res = backend.strategy.redirect(url)
    # payload = jwt_payload_handler(user)
    # res.set_cookie("name",user.name if user.name else user.username,max_age=24*3600)
    # res.set_cookie("token",jwt_encode_handler(payload),max_age=24*3600)
